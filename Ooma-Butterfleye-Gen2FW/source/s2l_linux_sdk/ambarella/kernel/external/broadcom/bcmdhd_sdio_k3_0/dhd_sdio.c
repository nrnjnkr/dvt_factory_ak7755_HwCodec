 ...
 */
#define TXGLOM_BUCKET_SIZE_1680 1680
#define TXGLOM_BUCKET_SIZE_1684 1684
	...
	if (bus->glom_enable) {
		...
		if (queue_only) {
			...
			if(bus->glom_cnt == 0) {
				...
				if (dhd_bus_chip_id(bus->dhd) == BCM43362_CHIP_ID ||
						dhd_bus_chip_id(bus->dhd) == BCM4330_CHIP_ID)
					len = TXGLOM_BUCKET_SIZE_1680 - TXGLOM_RECV_OFFSET;
				else
					len = TXGLOM_BUCKET_SIZE_1684 - TXGLOM_RECV_OFFSET;
			} else {
				// add pad to bucket size
				if (dhd_bus_chip_id(bus->dhd) == BCM43362_CHIP_ID ||
						dhd_bus_chip_id(bus->dhd) == BCM4330_CHIP_ID)
					len = TXGLOM_BUCKET_SIZE_1680;
				else
					len = TXGLOM_BUCKET_SIZE_1684;
			}
#else
...
dhd_bus_txdata(struct dhd_bus *bus, void *pkt)
{
	...
	dump_data += 4; /* skip 4 bytes bdc header */
...
dhdsdio_chipmatch(uint16 chipid)
{
	...
	if (chipid == BCM43438_CHIP_ID) {
		DHD_ERROR(("BCM43438 matched\n"));
		return TRUE;
	} else {
		DHD_ERROR(("BCM43438 not matched, 0x%x != 0x%x\n", chipid, BCM43438_CHIP_ID));
	}
	return FALSE;
}
